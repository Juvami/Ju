...
Animation loadAnimation(int sizeX, int sizeY, char *imgBMPSrc, long timePerFrame) {
    /** @brief charge une animation dans une structure Animation
     *  @param sizeX largeur en pixel d'un sprite de l'animation
     *  @param sizeX hauteur en pixel d'un sprite de l'animation
     *  @param imgBMPSrc le chemin vers la spritesheet de l'animation
     *  @param timePerFrame la durée en microseconde entre chaque changement de frame/sprite
     *  @return animation structure contenant les informations pour afficher une animation
     */
    Animation animation;
    ...
    if (!(animation.surfaceImage = SDL_LoadBMP(imgBMPSrc))) {
        SDL_Log("ERREUR : chargement img > %s\nParametres passes %s\n",SDL_GetError(), imgBMPSrc);
        freeAndTerminate();
    }
    animation.timePerFrame = timePerFrame;
    animation.nbrFrame = (animation.surfaceImage->w)/animation.sizeX;
    return animation;
}

void destroyAnimation(Animation animation) {
    SDL_FreeSurface(animation.surfaceImage);
}
...
Animation drawAnimation(int posX, int posY, long timeDiff, Animation animation) {
     ...
     *  @param posX position sur l'axe horizontale du coin supérieur gauche de l'image
     *  @param posY position sur l'axe verticale du coin supérieur gauche de l'image
     *  @param timeDiff le temps passée entre 2 dessin
     *  @param animation l'animation a affichée
     *  @return l'animation mise a jour avec la différence de temps
     */

     // on met a jour l'animation
    animation.currentTime -= timeDiff;
    if (animation.currentTime < 0) {
        animation.currentTime += animation.timePerFrame;
        animation.currentFrame++;
        animation.currentFrame = (animation.currentFrame < animation.nbrFrame)? animation.currentFrame:0;
    }

    // on extrait le sprite actuel
    SDL_Rect rectangle;
    rectangle.x = animation.currentFrame*animation.sizeX;
    rectangle.y = 0;
    rectangle.h = animation.sizeY;
    rectangle.w = animation.sizeX;

    SDL_Texture *textureAnim = NULL;
    // on doit créer une surface vierge avec un masque pour pouvoir gérer la transparence du sprite extrait
    SDL_Surface *surfaceAnim = SDL_CreateRGBSurface(0, rectangle.w, rectangle.h, 32, 0xff000000, 0x00ff0000, 0x0000ff00, 0x000000ff);
    if (SDL_BlitSurface(animation.surfaceImage, &rectangle, surfaceAnim, NULL)) {
        SDL_Log("ERREUR : decoupe Image > %s\n",SDL_GetError());
        freeAndTerminate();
    }
    textureAnim = SDL_CreateTextureFromSurface(renderer, surfaceAnim);
    SDL_FreeSurface(surfaceAnim);
    if (textureAnim == NULL) {
        SDL_Log("ERREUR : chargement texture > %s\nParametres passes %d , %d\n",SDL_GetError(), posX, posY);
        freeTexture(textureAnim);
        freeAndTerminate();
    }
    rectangle.x = posX;
    rectangle.y = posY;
    if (SDL_RenderCopy(renderer, textureAnim, NULL, &rectangle) != 0) {
        SDL_Log("ERREUR: image : RenderCopy > %s\nParametres passes %d , %d\n",SDL_GetError(), posX, posY);
        freeTexture(textureAnim);
        freeAndTerminate();
    }
