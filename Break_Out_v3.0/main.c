...
void init_game()
/*initialisation variables diverses + tableaux*/
{
...
void drawGame()
/*gestion de l'affichage graphique : dessine le jeu*/
{
    clear();
...
/*gestion des events manette : NON ACTIF*/
/*{
    switch (touche) {
        case SDLK_SPACE:
            break;
...
void gameLoop() 
/*gestion de la boucle de jeu. Répète tant que pas de Game Over*/
{
...
//SPRITES & ANIMATIONS
int liven_upBrick()
/*définition texture des briques*/
{
    //sprite_sheet pour Pokémons
    //éventuellement sprite_sheet pour Grass (vague, mouvement vent dans herbe)
...
int liven_upBall()
/*définition texture de la balle*/
{
...
int liven_upPaddle()
/*définition texture du paddle + semi-animation*/
{
...
// MVT PADDLE
double holdKey() 
/*détection maintien de touche enfoncée > accélération paddle + balle*/
{
	//printf("holdKey\n");
...
double goRight() 
/*déplacement paddle sur la droite*/
{
	//printf("Right HERE\n");
	psyduck.flip = 1;
	if (psyduck.x2 < FEUROUGE) {
...
// MVT BALL
double moveBall()
/*déplacement balle : vitesse + direction*/
{
	//printf("moveBall\n");
...
// GESTION BRIQUES
double collideBrick()
/*gestion des collisions balle/briques : changement vitesse + direction*/
{
	int i; int j;
	int roll_the_dice; //variable pour gérer des évènements aléatoires
...
/*détection des impacts balle/briques*/

/*x = bord gauche, xbis = bord droit -- y = bord sup, ybis = bord inf*/
{
...
// ANNEXES
int GameOver()
/*gestion de la sortie de balle + affichage Game Over*/
{
...
/*paddle avec des points de vie > doit récupérer des bonus pour rester en vie : NON FONCTIONNEL*/
/*{
